...
def test_get_mask_card_number_invalid_characters() -> None:
    with pytest.raises(ValueError):
        get_mask_card_number("abcd1234efgh5678")  # содержит не 16 цифр после удаления нецифровых символов
...
def test_get_mask_account_invalid_length() -> None:
    with pytest.raises(ValueError):
        get_mask_account("12")  # менее чем 4 цифр


def test_get_mask_account_no_digits() -> None:
    with pytest.raises(ValueError):
        get_mask_account("abcd")  # 0 цифр


def test_get_mask_account_invalid_characters() -> None:
    with pytest.raises(ValueError):
        get_mask_account("1234xyz")  # содержит нецифровые символы
